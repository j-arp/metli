class Ckeditor::AttachmentFilesController < Ckeditor::ApplicationController
  ...
  def index
    @attachments = Ckeditor.attachment_file_adapter.find_all(ckeditor_attachment_files_scope(assetable_id: active_story.id, assetable_type: "Story"))
    @attachments = Ckeditor::Paginatable.new(@attachments).page(params[:page])
    respond_with(@attachments, :layout => @attachments.first_page?)
  end

  def create
    @attachment = Ckeditor.attachment_file_model.new(assetable_id:  active_story.id, assetable_type: "Story")
